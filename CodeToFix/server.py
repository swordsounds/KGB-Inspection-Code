...
def get_command():
    ...
    uart = serial.Serial('/dev/ttyS0', baudrate = 115200, parity=serial.PARITY_NONE, stopbits=serial.STOPBITS_ONE, bytesize=serial.EIGHTBITS)
    ...
    while True:
        try:
            ...
            print('',end='\n')
            controller.execute_command(command=frame, ser=uart)
...
class Controller():
    ...
    def __init__(self, queue=None, **kwargs):
        ...
        #in1_pwm = io.PWM(in1,200)
        #in2_pwm = io.PWM(in2,200)
        #in3_pwm = io.PWM(in3,200)
        #in4_pwm = io.PWM(in4,200)
        #in1_pwm.start(90)
        #in2_pwm.start(90)
        #in3_pwm.start(90)
        #in4_pwm.start(90)
    ...
    def execute_command(self, command, ser):
        ...
        if pwm1 > 0 and pwm4 > 0:
            ser.write(b'N1 D500 d500 A200 G3 \n')
            
        
        if pwm2 > 0 and pwm3 > 0:
            ser.write(b'N1 D-500 d-500 A200 G3 \n')
        

        if pwm1 > 0 and pwm3 > 0:
            ser.write(b'N1 D500 d-500 A200 G3 \n')
        

        if pwm2 > 0 and pwm4 > 0:
            ser.write(b'N1 D-500 d500 A200 G3 \n')

        if pwm1 == 0 and pwm2 == 0 and pwm3 == 0 and pwm4 == 0:
            ser.write(b'N1 D0 d0 A200 G3 \n')
